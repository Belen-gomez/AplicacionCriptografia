...
def reservar(correo_usuario, nombre):
    origen = input("¿Dónde quieres empezar tu viaje? ").lower()
    destino = input("¿A dónde quieres ir? ").lower()
...
def ver_viajes(data_list, correo_usuario):
    path = "/DATOS/BELÉN/3º UNI/Criptografía/Practica_1/Criptografia/usuarios/" + correo_usuario + "/key.pem"
...
path = "/DATOS/BELÉN/3º UNI/Criptografía/Practica_1/Criptografia/usuarios/" + correo_usuario + "/viajes.json"
try: 
    with open(path, "r", encoding="utf-8", newline="") as file:
        data_list = json.load(file)
        respuesta = input("Quieres reservar un nuevo viaje (R) o ver tus viajes ya reservados (V)").lower()
        if respuesta == "r":
            reservar(correo_usuario, nombre)
        elif respuesta == "v":
            ver_viajes(data_list, correo_usuario)
